 ...
 *  CRC-32C Checksum
 *
 *  The code is based on sources from KAME Project http://www.kame.net
 *  See kame/kame/sys/netinet/sctp_crc32.c
 *
 *  See also http://tools.ietf.org/html/rfc4960
 */
#include <cuttle/hash/crc32.h>
#include <endian.h>
#include <string.h>

#define CRC32BASE         0xffffffff
...
{
  return cf_crc32_update(crc, buff, strlen(buff));
}
...
uint32_t cf_crc32_finalize(uint32_t crc)
{
  ...
  /* For BIG-ENDIAN.. aka Motorola byte order the result is in
     * little-endian form. So we must manually swap the bytes. */
  return htobe32(crc);
#endif
}
